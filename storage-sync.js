...
async function pullFileFromSupabase(slot) {
  ...
  // ── PERF FIX: cache-first load ──────────────────────────────────────
  // Before touching the network at all, check IndexedDB for whatever this
  // slot's file looked like at the end of the LAST session. If present,
  // load it into the input immediately — this is what lets the app show
  // real data on launch in well under a second, even on a slow connection
  // or offline, instead of a blank/loading page until the download
  // finishes. We still go on to check Supabase's metadata below and only
  // re-download the actual file if it has genuinely changed since — the
  // cached copy is just what's shown WHILE that check happens.
  //
  // Safe no-op if IndexedDB is unavailable (EpssIdbCache fails soft) or
  // this is the very first time this browser has ever loaded the slot.
  const idb = window.EpssIdbCache;
  let cached = null;
  if (idb) {
    try { cached = await idb.get(slot); } catch (e) { cached = null; }
    if (cached && cached.blob) {
      await dispatchFileToInput(slot, cached.blob, cached.filename || path.split("/").pop());
    }
  }
  ...
  // caching behavior.
  //
  // FIX-CACHE-SKIP: fetch the row metadata FIRST (cheap — a small table
  // read, not a file download) and compare its uploaded_at against what we
  // just loaded from IndexedDB. If they match, the cached copy IS current —
  // skip the (potentially large) file re-download entirely. This is the
  // "only re-download a slot when the remote file has actually changed"
  // requirement; the two file-slot performance wins compound (instant
  // paint from cache + zero wasted bandwidth when nothing changed).
  const remoteMeta = await fetchSlotMetadata(slot);
  const remoteStamp = remoteMeta && remoteMeta.uploadedAt ? remoteMeta.uploadedAt.toISOString() : null;
  if (cached && cached.uploadedAt && remoteStamp && cached.uploadedAt === remoteStamp) {
    slotMeta[slot] = remoteMeta;
    renderSyncInfo(slot);
    return; // cache already matches the server — nothing more to do
  }
  ...
  if (signErr) {
    ...
    // Only clear the "last known" indicator if we don't already have a
    // cached copy on screen — a transient network/signing error shouldn't
    // make a viewer's already-loaded cached data look unsynced.
    if (!cached) { slotMeta[slot] = null; renderSyncInfo(slot); }
    return;
  }
  ...
  await dispatchFileToInput(slot, blob, filename);

  // PERF FIX: remember this exact copy (blob + its uploaded_at stamp) so
  // next launch can show it instantly via the cache-first path above,
  // and skip re-downloading entirely if it's still current by then.
  if (idb) {
    try {
      await idb.set(slot, { uploadedAt: remoteStamp, filename, blob });
    } catch (e) { /* best-effort — never blocks the UI on a cache-write failure */ }
  }

  slotMeta[slot] = remoteMeta || (await fetchSlotMetadata(slot));
  renderSyncInfo(slot);
}

// ── Shared helper: populate the real <input type=file> with a Blob/File
// so script.js/mos.js/shelf-life.js's EXISTING (unmodified) change
// listeners parse it exactly as if the user had picked it by hand. Used
// by both the IndexedDB cache-first path and the live Supabase download
// path above, so there is exactly one place that does this DOM dance.
async function dispatchFileToInput(slot, blob, filename) {
  const { inputId, statusId } = FILE_SLOTS[slot];
