...
(function wireStockoutRiskModule() {
  function extend() {
    ...
    // with its own unguarded branch for "stockout-risk", which bypassed the
    // canAccessModule() permission check that lives in the real renderPage()
    // (script.js) entirely — a user with zero permissions could open this
    // page directly. Registering into PAGE_RENDERERS is sufficient: the
    // real renderPage() already calls PAGE_RENDERERS[id]?.() once the
    // central permission gate has passed, so no separate wrapper is needed
    // (or safe to have).
