  ...
  reader.onload = e => {
    ...
    parseWorkbookOffThread(e.target.result, {
      ...
      try {
        ...
        // Normalize numeric/derived fields on EVERY trimmed row (not just the
        // plant-scoped `df` subset) — see FIX-BRD-CROSS-BRANCH-BLIND-SPOT
        // below, which needs these same derived fields (_expiry, Total Qty,
        // etc.) on rows that `df` itself excludes for a plant-locked user.
        // Mutating in place means df/rawDfNational below share identical,
        // already-normalized row objects — no duplicated parsing work.
        trimmed.forEach(row => {
        ...
        df = df.filter(hasAnyStock);

        rawDf  = df;
        filtDf = df;

        // ── FIX-BRD-CROSS-BRANCH-BLIND-SPOT ─────────────────────────────────
        // rawDf above is PLANT-scoped (via filterRowsByAccess → canAccessRow
        // → canAccessPlant) to whichever plant the SIGNED-IN user is locked
        // to — correct for anything that DISPLAYS rows to that user, but
        // wrong for calculations that are inherently cross-branch, like
        // Branch Demand's priority-tier allocation math (see
        // brdBuildNationalSohMap() in branch-demand.js and the matching
        // canAccessRowIgnoringPlant()/filterRowsByAccessIgnoringPlant() in
        // permissions.js). rawDfNational keeps the same data-scope +
        // universal-invariant filtering as rawDf, just never denies a row
        // for belonging to a plant this user can't otherwise see — so a
        // branch-locked user's copy of "every branch's real stock" matches
        // an HO01/Admin user's, and cross-branch math comes out identical
        // for both instead of silently treating other branches' SOH as 0.
        window.rawDfNational = filterRowsByAccessIgnoringPlant(trimmed).filter(hasAnyStock);
...
function renderPage(id) {
  ...
  const RAWDF_EXEMPT_PAGES = ["pending-dispatch", "mos-plant", "natl-table", "request-analysis", "branch-demand", "user-management"];
